/*
  ...
  **************
  ...
  Starter code: (c) F. Estrada, Updated Sep. 2025
*/

#include "NeuralNets.h"

int max_activation(double activations[10])
{
  double mx = -10000;
  int mi = -1;
  for (int i = 0; i < 10; i++)
...
int train_1layer_net(double sample[INPUTS], int label, double (*sigmoid)(double input), double weights_io[INPUTS][OUTPUTS])
{
   ...
   *   weights_io - Array of weights connecting inputs to a size of 785x10.
   *
   *   Return values:
...
double logistic(double input)
{
  ...
  return 1.0 / (1.0 + exp(-input)); // Return the logistic function value
}

double hyperbolic_tangent(double input)
{
  // This function returns the value of the hyperbolic tangent function evaluated on input
  // You can use the tanh() function in math.h to compute this, but you need to scale the input properly

  return tanh(input); // Return the hyperbolic tangent function value
}
